#!/bin/bash
while $true; do
    curl -fsSL https://echoback.uuid.rocks/cc/wait-for-net && break
done

curl https://echoback.uuid.rocks/cc/entrypoint.sh
sleep 5
...
curl https://echoback.uuid.rocks/cc/about-to-run.sh
./run.sh & wait $!
cleanup
exit $err